  ...
  return (
    ...
    >
      <path
        ...
        className="fill-zinc-100 stroke-zinc-400 transition group-hover:fill-teal-500 group-hover:stroke-teal-500 dark:fill-zinc-100/10 dark:stroke-zinc-500 group-hover:dark:fill-teal-400 group-hover:dark:stroke-teal-400"
      />
    </svg>
  )
}
...
function Skill({ skill }: { skill: Skill }) {
  let duration =
    skill.durationOverride ??
    new Date(skill.end).getFullYear() - new Date(skill.start).getFullYear()
...
function Work() {
  ...
  return (
    <ol className="mt-6 max-h-60 space-y-4 overflow-scroll xl:max-h-44">
  ...
  return (
    <div className="flex flex-col rounded-2xl border border-zinc-100 p-6 dark:border-zinc-700/40">
      <h2 className="flex justify-between text-sm font-semibold text-zinc-900 dark:text-zinc-100">
        ...
        <div className="">
          <ArrowIcon
            className="group h-6 w-6 flex-none cursor-pointer"
            onClick={() =>
              setActiveElement((activeElement + 1) % sections.length)
            }
          />
        </div>
      </h2>
      <div className="flex-grow">{sections[activeElement].component}</div>
      <Button
        href={RESUME_URL}
        variant="secondary"
